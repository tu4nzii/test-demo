...
async def run_dataset_segmentwise_feedback(
    ...
    inner_radius = int(cfg.get("r_pixels") or min(w, h) // 4)
    outer_radius = int(cfg.get("r_pixels") or min(w, h) // 4)
    ...
    for point in cfg["data_points"].keys():
        ...
        for round_idx in range(repeat_rounds):
            ...
            gt_pct = cfg["data_points"][point]/360
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Pie chart batch evaluator")
