"""Model calls for vertical bar charts."""
...
from prediction_core.chart_io import image_to_data_url
from prediction_core.json_utils import parse_model_json, unwrap_openai_content
...
class VBarModelClient:
    ...
    async def __aenter__(self) -> "VBarModelClient":
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            try:
                async with session.post(url, headers=get_headers(), json=self._payload(prompt, image_path)) as resp:
                    ...
                    if resp.status != 200:
                        print(f"[v_bar model] HTTP {resp.status}: {text[:200]}")
                        await asyncio.sleep(2 * attempt)
                        continue
                    return unwrap_openai_content(text)
            except (aiohttp.ClientError, asyncio.TimeoutError) as exc:
                print(f"[v_bar model] attempt {attempt}/{self.max_retries} failed: {exc}")
                await asyncio.sleep(2 * attempt)
        return None
