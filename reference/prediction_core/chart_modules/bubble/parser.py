"""Model-output parsing for bubble charts."""
...
from reference.prediction_core.json_utils import safe_json_loads as core_safe_json_loads
...
def extract_coords(coords_json: Any, point_name: str) -> tuple[Any, Any]:
    if coords_json is None:
        print("[bubble parser] Empty JSON payload.")
        return FAILED_COORDS
    print("[bubble parser] input=" f"{type(coords_json).__name__} {json.dumps(coords_json, ensure_ascii=False)[:200]}")
    result = _extract(coords_json, point_name)
    if result is None:
        print(f"[bubble parser] No matching coordinates found for {point_name!r}")
        return FAILED_COORDS
    return result
