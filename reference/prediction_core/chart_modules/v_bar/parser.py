"""Model-output parsing for vertical bar value prediction."""
...
def extract_coords(coords_json: Any, point_name: str) -> tuple[Any, Any]:
    if coords_json is None:
        print("[v_bar parser] Empty JSON payload.")
    ...
    if result is None:
        print(f"[v_bar parser] No matching coordinates found for {point_name!r}")
        return FAILED_COORDS
    return result
