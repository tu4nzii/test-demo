...
def _color_context(item_name: str, series_color: dict[str, Any] | None) -> str:
    ...
    if target_color:
        lines.append(
            f'Before reading angles, follow the text label/leader line for "{item_name}" and select the continuous ring sector with color {target_color}.'
        )
        lines.append(
...
def generate_prompt(
    ...
    printed_value_rule = f"""
        ...
        - If numeric data labels are printed inside or near the donut sectors, read those numbers first.
    ...
    # —— amplifier ——
        ...
        Critical guardrail: do not report the small complementary wedge unless that wedge is truly the sector labeled "{item_name}". A labeled sector may be larger than 50% of the donut.
        Instructions:
        ...
        - Also estimate the percentage share directly from the visible ring sector area. Use percentage as the primary value; use angles only as supporting evidence.
        ...
        prompt = f"""
                    ...
                    To support estimation, this chart includes **radial reference lines every 15°**. Angle convention: 0° points upward at 12 o'clock, and angles increase clockwise.
                    ...
                    Critical guardrail: if the visible label/color of "{item_name}" occupies the larger part of the donut, return a percentage greater than 50; do not replace it with the smaller complementary slice.
                    {color_context}
                    ...
                    4. Estimate the percentage share directly from the visible ring sector area. Use percentage as the primary value; use angles only as supporting evidence.
                    Output *only*:
        ...
        prompt = f"""
                ...
                Angle convention: 0° points upward at 12 o'clock, and angles increase clockwise as follows:
                ...
                The sector may cross 0°, so start_angle may be greater than end_angle.
                Critical guardrail: first identify the text label and its colored ring sector, then estimate that region's area share. Do not report the complement. A sector may exceed 50%.
