...
from ...common.amplifier_style import AMPLIFIER_GRID_COLOR_RGBA, amplifier_max_side, amplifier_target_side
...
PREFERRED_PROMPTS = ["amplifier", "feedback", "grid"]
CHART_TYPE = "pie"
...
async def _run_baseline_target(dataset: dict[str, Any], target: CircularTarget) -> list[dict[str, Any]]:
    ...
    print(f"[pie] Baseline | Segment: {target.point_name}")
    token = set_modal_call_context(
...
async def _run_refined_target(
    ...
    center, radius = _pie_geometry(dataset, no_grid_image)
    ...
    print(f"[pie] {chart_id} | {target.point_name} | grid round 1")
    token = set_modal_call_context(
    ...
    feedback_rounds = get_feedback_rounds(2)
    for feedback_round in range(1, feedback_rounds + 1):
        try:
        ...
        print(f"[pie] {chart_id} | {target.point_name} | feedback round {feedback_round}")
        token = set_modal_call_context(
        ...
        if angle_prediction_stable(prediction, previous_prediction):
            print(f"[pie] {chart_id} | {target.point_name} | feedback stabilized within 2 deg; enter amplifier.")
    ...
    for amp_round in range(1, get_amplifier_rounds(3) + 1):
        ...
        if is_stable:
            print(f"[pie] {chart_id} | {target.point_name} | amplifier prediction stabilized at round {amp_round}; stop refinement.")
            break
...
def _pie_geometry(dataset: dict[str, Any], source_image: Path) -> tuple[tuple[int, int], int]:
    ...
    if isinstance(raw_radius, (list, tuple)):
        ...
        radius = max(valid_radii) if valid_radii else int(min(width, height) * 0.35)
    else:
...
def _add_target_color_swatch(path: Path, dataset: dict[str, Any], point_name: str) -> None:
    ...
    try:
        ...
        print(f"[pie] Target color swatch skipped for {point_name}: {exc}")


def _crop_sector_for_prediction(
    *,
    source: Path,
    out_path: Path,
    ...
    with Image.open(source).convert("RGBA") as base:
        bbox = _sector_bbox(center, radius, base.size)
        crop = base.crop(bbox)
        local_center = (center[0] - bbox[0], center[1] - bbox[1])
        result = crop.copy()
        _draw_sector_amplifier_guides(
            result,
        ...
        short_side = max(1, min(result.size))
        target_side = amplifier_target_side(family="sector")
        if short_side < target_side:
            scale = target_side / short_side
            result = result.resize(
                (int(round(result.width * scale)), int(round(result.height * scale))),
...
def _draw_sector_amplifier_guides(
    image: Image.Image,
    *,
    ...
    for angle in (start_angle, end_angle):
        label = "start" if angle == start_angle else "end"
        theta = math.radians(angle - 90.0)
        x = cx + radius * math.cos(theta)
        y = cy + radius * math.sin(theta)
        draw.line([(cx, cy), (x, y)], fill=AMPLIFIER_GRID_COLOR_RGBA, width=4)
        marker = max(4, int(radius * 0.018))
        draw.line([(x - marker, y), (x + marker, y)], fill=AMPLIFIER_GRID_COLOR_RGBA, width=3)
        draw.line([(x, y - marker), (x, y + marker)], fill=AMPLIFIER_GRID_COLOR_RGBA, width=3)
        try:
            draw.text((x + marker + 4, y + marker + 4), f"{label} {angle:.1f}", fill=AMPLIFIER_GRID_COLOR_RGBA)
        except Exception:
            pass

    bbox = (cx - radius, cy - radius, cx + radius, cy + radius)
    start_draw = (start_angle - 90.0) % 360.0
    end_draw = (end_angle - 90.0) % 360.0
    if start_draw <= end_draw:
        draw.arc(bbox, start=start_draw, end=end_draw, fill=AMPLIFIER_GRID_COLOR_RGBA, width=4)
    else:
        draw.arc(bbox, start=start_draw, end=360, fill=AMPLIFIER_GRID_COLOR_RGBA, width=4)
        draw.arc(bbox, start=0, end=end_draw, fill=AMPLIFIER_GRID_COLOR_RGBA, width=4)
