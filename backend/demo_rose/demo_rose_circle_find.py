...
class RoseChartEncoder:
    ...
    def encrypt_rose_chart_with_tick(self, image_path, tick_interval, tick1, tick2, max_tick_value, min_tick_value):
        ...
        while tick > 0 and radius >= 0:
            ...
            if radius <= 0:
                print(f"鈿狅笍 璺宠繃鏃犳晥鍗婂緞: tick={tick}, 璁＄畻鍗婂緞={radius}")
                continue
    ...
    def process_single_image(self, image_path, output_dir=None):
        ...
        try:
            ...
            print(f"绗竴涓渾鍒诲害璇嗗埆: {reason1}")
            print(f"绗竴涓渾鍒诲害鍊? {tick1}")
            print(f"绗簩涓渾鍒诲害璇嗗埆: {reason2}")
            print(f"绗簩涓渾鍒诲害鍊? {tick2}")
            ...
            print(f"LLM鍒嗘瀽缁撴灉: {tick1}, {tick2}, {max_tick_value}, {res}")
            ...
            if os.path.exists(json_path):
                ...
                print(f"鏇存柊鍚庣殑JSON宸蹭繚瀛樿嚦: {output_json_path}")
            else:
                print(f"JSON鏂囦欢鏈壘鍒? {json_path}")
...
if __name__ == "__main__":
    ...
    image_path = "./data/rose/rose_001.png"  # 鍙互鏍规嵁闇€瑕佷慨鏀?
    output_dir = "./data/output/rose"      # 鍙互鏍规嵁闇€瑕佷慨鏀?
