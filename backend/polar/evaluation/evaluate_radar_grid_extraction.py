...
SYNTH_RADAR_DIR = POLAR_DATA / "output" / "axis_sample_selection" / "radar_rose_50charts_20260628_140358" / "radar"
OUTPUT_DIR = POLAR_DATA / "output" / "radar_grid_eval"
...
def resolve_image(json_path: Path, data: dict[str, Any]) -> Path | None:
    ...
    if isinstance(direct_image, str) and direct_image:
        ...
        fallback_candidates.append(ROOT / direct_image)
        fallback_candidates.append(BACKEND / direct_image)
    for key in ("no_grid", "image", "with_grid"):
        value = image_paths.get(key)
        if isinstance(value, str) and value:
            path = Path(value)
            candidates.append(path if path.is_absolute() else json_path.parent / path)
            fallback_candidates.append(ROOT / value)
            fallback_candidates.append(BACKEND / value)
...
def iter_dataset_jsons(dataset: str, synth_dir: Path | None = None) -> list[Path]:
    ...
    paths = []
    source_dir = synth_dir or SYNTH_RADAR_DIR
    for path in sorted(source_dir.glob("radar_*.json")):
...
def origin_consistency_reason(matches: list[dict], short_side: int) -> str:
    """Reject radius-tick mappings whose fitted zero tick is not at the center.

    The gate uses only the detector's radius/tick association.  For ordinary
    polar/radar charts, the radial mapping should extrapolate close to
    ``tick=0 -> pixel radius=0``.  A large positive intercept means the detected
    rings are shifted away from the chart origin, which is the failure mode seen
    in RadarChart19.
    """
    unique: dict[float, float] = {}
    for match in matches:
        tick = match.get("gt_tick")
        radius = match.get("pred_radius")
        if tick is None or radius is None:
            continue
        tick_value = float(tick)
        radius_value = float(radius)
        if tick_value <= 0 or radius_value <= 0:
            continue
        unique.setdefault(tick_value, radius_value)

    if len(unique) < 2:
        return ""

    ticks = np.array(sorted(unique), dtype=float)
    radii = np.array([unique[tick] for tick in ticks], dtype=float)
    slope, intercept = np.polyfit(ticks, radii, deg=1)
    if not math.isfinite(float(slope)) or slope <= 0 or not math.isfinite(float(intercept)):
        return "center_not_at_origin_zero_tick_line_not_through_center"

    threshold = max(8.0, short_side * 0.012)
    ...
    try:
        ...
        if not fallback_reason and pred_center:
            ...
            if not fallback_reason:
                ...
                origin_reason = origin_consistency_reason(matches, short_side)
                if origin_reason:
                    fallback_reason = origin_reason
                    notes.append(origin_reason)
                ...
                tolerance_pass = bool((not fallback_reason) and checked and max(checked) <= tolerance_px)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        dest="synthetic_dir",
        type=Path,
        help="Directory containing synthetic radar_*.json/images.",
    )
...
def main() -> int:
    ...
    for dataset in selected:
        paths = iter_dataset_jsons(dataset, args.synthetic_dir)
        if args.only:
