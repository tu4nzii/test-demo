...
class RadarChartEvaluator:
    def __init__(self):
        ...
        self.amplifier_feedback_image_dir = os.path.join(self.amplifier_image_dir, 'feedback')
        self.amplifier_grid_image_dir = os.path.join(self.amplifier_image_dir, 'grid')
    ...
    def _create_directories(self):
        ...
        if not os.path.exists(self.amplifier_feedback_image_dir):
            os.makedirs(self.amplifier_feedback_image_dir)
        if not os.path.exists(self.amplifier_grid_image_dir):
            os.makedirs(self.amplifier_grid_image_dir)
    ...
    def normalize_dataset(self, dataset: dict) -> dict:
        """Normalize real/legacy radar JSON into this evaluator's nested data shape."""
        if not isinstance(dataset, dict):
            return {}

        if 'data' not in dataset and isinstance(dataset.get('data_points'), dict):
            dataset['data'] = dataset['data_points']

        data = dataset.get('data')
        if isinstance(data, dict) and data and not all(isinstance(value, dict) for value in data.values()):
            nested = {}
            for key, value in data.items():
                entity_name, axis_label = self.split_flat_radar_key(str(key), dataset)
                if entity_name and axis_label:
                    nested.setdefault(entity_name, {})[axis_label] = value
            dataset['data'] = nested

        if not isinstance(dataset.get('axis_labels'), dict):
            theta_ticks = dataset.get('theta_ticks') or dataset.get('labels') or []
            theta_angles = dataset.get('theta_angles') or dataset.get('axes_angles') or []
            dataset['axis_labels'] = self.build_axis_labels(theta_ticks, theta_angles)
        else:
            # Some JSONs store axis_labels reversed as {label: angle} → normalize to {angle: label}
            axis_labels = dataset['axis_labels']
            if axis_labels:
                first_key = next(iter(axis_labels))
                try:
                    float(first_key)
                except (ValueError, TypeError):
                    # Keys are labels, not angles → reversed format → rebuild from source arrays
                    theta_ticks = dataset.get('theta_ticks') or dataset.get('labels') or list(axis_labels.keys())
                    theta_angles = dataset.get('theta_angles') or dataset.get('axes_angles') or list(axis_labels.values())
                    dataset['axis_labels'] = self.build_axis_labels(theta_ticks, theta_angles)

        return dataset

    def get_axis_label_candidates(self, dataset: dict) -> List[str]:
        labels = []
        labels.extend(str(value) for value in dataset.get('theta_ticks', []) or [])
        labels.extend(str(value) for value in dataset.get('labels', []) or [])
        axis_labels = dataset.get('axis_labels', {})
        if isinstance(axis_labels, dict):
            labels.extend(str(value) for value in axis_labels.values())
        return sorted(set(label for label in labels if label), key=len, reverse=True)

    def split_flat_radar_key(self, key: str, dataset: dict) -> Tuple[str, str]:
        for label in self.get_axis_label_candidates(dataset):
            for separator in ('_', ',', ', '):
                suffix = f"{separator}{label}"
                if key.endswith(suffix):
                    return key[:-len(suffix)].strip(), label

        if '_' in key:
            entity_name, axis_label = key.rsplit('_', 1)
            return entity_name.strip(), axis_label.strip()
        if ',' in key:
            entity_name, axis_label = key.rsplit(',', 1)
            return entity_name.strip(), axis_label.strip()
        return '', ''

    def build_axis_labels(self, theta_ticks: List, theta_angles: List) -> Dict[str, str]:
        axis_labels = {}
        for label, angle in zip(theta_ticks, theta_angles):
            try:
                key = str(int(round(float(angle))))
            except Exception:
                key = str(angle)
            axis_labels[key] = str(label)
        return axis_labels

    def axis_label_to_angle(self, axis_labels: Dict) -> Dict[str, int]:
        label_to_angle = {}
        for angle, label in axis_labels.items():
            try:
                label_to_angle[str(label)] = int(round(float(angle)))
            except Exception:
                continue
        return label_to_angle

    def chart_angle_to_image_angle(self, angle: float) -> float:
        """Convert radar chart angle convention to image-coordinate angle."""
        return (90 - float(angle)) % 360

    def split_item_name(self, item_name: str, dataset: dict = None) -> Tuple[str, str]:
        dataset = dataset or {}
        for label in self.get_axis_label_candidates(dataset):
            for separator in ('_', ',', ', '):
                suffix = f"{separator}{label}"
                if item_name.endswith(suffix):
                    return item_name[:-len(suffix)].strip(), label

        if '_' in item_name:
            entity_name, axis_label = item_name.rsplit('_', 1)
            return entity_name.strip(), axis_label.strip()
        if ',' in item_name:
            entity_name, axis_label = item_name.rsplit(',', 1)
            return entity_name.strip(), axis_label.strip()
    ...
    def resolve_series_color(self, entity_name: str, series_color: Dict[str, str]) -> Tuple[str, str]:
        """Return the legend entity name and color, tolerating small label typos."""
        if not isinstance(series_color, dict) or not series_color:
            return entity_name, '未知颜色'
        if entity_name in series_color:
            return entity_name, series_color[entity_name]

        normalized_target = self.normalize_series_name(entity_name)
        for legend_name, color in series_color.items():
            if self.normalize_series_name(legend_name) == normalized_target:
                return legend_name, color

        best_name = ''
        best_score = 0.0
        for legend_name in series_color:
            score = SequenceMatcher(
                None,
                normalized_target,
                self.normalize_series_name(legend_name)
            ).ratio()
            if score > best_score:
                best_name = legend_name
                best_score = score

        if best_name and best_score >= 0.88:
            return best_name, series_color[best_name]
        return entity_name, '未知颜色'
    ...
    def encode_image(self, image, center_x: int, center_y: int, arg_a: float, arg_b: float,
                     r_ticks: List[float], min_pixel_gap: int = None) -> np.ndarray:
        """在图像上绘制加密网格线（自适应密度）。min_pixel_gap=None时自动计算。"""
        line_color = (180, 180, 180)
        thickness = 1
        if r_ticks and r_ticks[0] == 0:
            r_ticks.pop(0)

        if min_pixel_gap is None:
            radii = [int(arg_a * t + arg_b) for t in r_ticks if int(arg_a * t + arg_b) > 0]
            if len(radii) >= 2:
                gaps = [radii[i+1] - radii[i] for i in range(len(radii)-1)]
                avg_gap = np.mean(gaps) if gaps else 30
                min_pixel_gap = max(10, min(30, int(avg_gap * 0.4)))
            else:
                min_pixel_gap = 15
        print(f"  [encode_image] auto min_pixel_gap={min_pixel_gap}px (from {len(r_ticks)} ticks, arg_a={arg_a:.3f})")

        count = 0
        prev_radius = -min_pixel_gap
        for tick in r_ticks:
            radius = int(arg_a * tick + arg_b)
            if radius <= 0:
                continue
            if radius - prev_radius < min_pixel_gap:
                continue
            count += 1
            if count % 4 == 0 and radius - prev_radius >= min_pixel_gap * 2:
                prev_radius = radius
                continue
            prev_radius = radius
    ...
    def crop_axis_label_region(self, image_path: str, center_x: int, center_y: int, angle_deg: float, 
        ...
        # 在图像上添加角度文字标注 (动态字体)
        sector_span_deg = end_angle - start_angle
        sector_height_est = outer_radius * math.tan(math.radians(max(sector_span_deg, 10) / 2)) * 2
        target_font_px = max(8, min(16, int(sector_height_est * 0.06)))
        font = cv2.FONT_HERSHEY_SIMPLEX
        font_scale = target_font_px / 25.0
        font_color = (0, 0, 0)
    ...
    def crop_axis_centered_strip(self, image_path: str, center_x: int, center_y: int, angle_deg: float,
                                 outer_radius: int, angle_width: int = 30, inner_radius: int = 0,
                                 label_offset: int = 30, scale_factor: float = 1.0,
                                 r_ticks: List[float] = None, arg_a: float = 0, arg_b: float = 0) -> np.ndarray:
        """Crop a zoom strip where the selected axis runs through the center."""
        if r_ticks is None:
            r_ticks = []
        ...
        radius_values = [
            int(round(arg_a * tick + arg_b))
            for tick in r_ticks
            if arg_a * tick + arg_b >= 0
        ]
        max_tick_radius = max(radius_values) if radius_values else outer_radius
        effective_outer = max(outer_radius, max_tick_radius)
        x_pad = max(18, int(label_offset * 0.6))
        y_half = max(28, min(95, int(effective_outer * math.tan(math.radians(max(angle_width, 12) / 2)) * 0.45)))

        x_start = max(0, int(center_x + max(inner_radius, 0) - x_pad))
        x_end = min(w, int(center_x + effective_outer + x_pad))
        y_start = max(0, int(center_y - y_half))
        y_end = min(h, int(center_y + y_half))
        if x_end <= x_start or y_end <= y_start:
            return image

        crop_img = rotated[y_start:y_end, x_start:x_end].copy()
        axis_y = int(center_y - y_start)

        if scale_factor != 1.0 and crop_img.size > 0:
            new_width = int(crop_img.shape[1] * scale_factor)
            new_height = int(crop_img.shape[0] * scale_factor)
            crop_img = cv2.resize(crop_img, (new_width, new_height), interpolation=cv2.INTER_LINEAR)
            axis_y = int(round(axis_y * scale_factor))

        cv2.line(crop_img, (0, axis_y), (crop_img.shape[1] - 1, axis_y), (200, 200, 200), 1, lineType=cv2.LINE_AA)

        # 动态字体缩放
        strip_height = crop_img.shape[0]
        target_font_px = max(10, min(20, int(strip_height * 0.13)))
        tick_font = cv2.FONT_HERSHEY_SIMPLEX
        tick_font_scale = target_font_px / 25.0
        tick_font_color = (0, 0, 0)
        tick_thickness = max(1, int(tick_font_scale * 1.5))

        for tick_index, tick in enumerate(r_ticks):
            radius = int(round(arg_a * tick + arg_b))
            tick_x = int(round((center_x + radius - x_start) * scale_factor))
            if tick_x < 0 or tick_x >= crop_img.shape[1]:
                continue
            text = str(int(tick) if tick % 1 == 0 else tick)
            text_size = cv2.getTextSize(text, tick_font, tick_font_scale, tick_thickness)[0]
            text_x = max(0, min(crop_img.shape[1] - text_size[0], tick_x - text_size[0] // 2))
            if tick_index % 2 == 0:
                text_y = max(text_size[1] + 1, axis_y - 4)
            else:
                text_y = min(crop_img.shape[0] - 2, axis_y + text_size[1] + 6)
            cv2.putText(crop_img, text, (text_x, text_y), tick_font, tick_font_scale,
                       tick_font_color, tick_thickness, lineType=cv2.LINE_AA)
    ...
    def generate_prompt(self, item_name: str, prompt_type: str, dataset: dict, tick: float = 0) -> str:
        ...
        item_entity, item_axis = self.split_item_name(item_name, dataset)
        legend_entity, legend_color = self.resolve_series_color(item_entity, dataset.get('series_color', {}))
        entity_color_hint = item_entity
        if legend_entity != item_entity:
            entity_color_hint = f"{item_entity} (legend label: {legend_entity})"
        series_color = dataset.get('series_color', {})
        if isinstance(series_color, dict) and item_entity not in series_color and legend_color != '未知颜色':
            series_color[item_entity] = legend_color
...
现在的局部放大图为{item_axis}轴对应的局部放大
您的任务是估计标记为"{item_name}"对应的值，即{item_name}对应实体颜色的数值。
请先找到{item_entity}对应实体颜色为{dataset.get('series_color', {}).get(item_entity, '未知颜色')}
然后找到该颜色对应的点，并插值出数值
...
'''.strip()
    ...
    def call_llm_response(self, prompt: str, image_path: str, item_name: str, dataset: dict) -> Tuple[Optional[float], Optional[float]]:
        ...
        while retry_count < max_retries:
            try:
                response = requests.post(url=self.url, headers=self.headers, json=payload, timeout=60)
    ...
    def process_single_image(self, json_path: str) -> None:
        ...
        max_points = int(os.getenv("REAL_POLAR_EVAL_MAX_POINTS", "0") or 0)
        processed_points = 0
        ...
        for entity_name, entity_data in dataset.get('data', {}).items():
            ...
            for axis_label, value in entity_data.items():
                if max_points and processed_points >= max_points:
                    return
                processed_points += 1
                ...
                for grid_type in ['with_grid', 'no_grid']:
                    ...
                    try:
                        ...
                        # 获取轴标签和角度映射
                        axis_labels = dataset.get('axis_labels', {})
                        label_to_angle = self.axis_label_to_angle(axis_labels)  # 建立标签到角度的反向映射
                        ...
                        target_angle = self.chart_angle_to_image_angle(label_to_angle[axis_label])
                        ...
                        if grid_type == 'with_grid':
                            ...
                            try:
                                ...
                                print(f"❌ 放大处理异常: {e}")
                            # Multi-step amplifier crops: amp_0 / amp_1 / amp_2.
                            try:
                                amplifier_path = dataset["image_paths"].get('no_grid', '').replace('\\', '/')
                                if not amplifier_path or not os.path.exists(amplifier_path):
                                    print(f"⚠️ 未找到no_grid图像路径或文件不存在: {amplifier_path}")
                                ...
                                max_radius = int(round(arg_a * r_ticks[-1] + arg_b))
                                min_tick_radius = int(round(arg_a * r_ticks[0] + arg_b))
                                radius_span = max(1, max_radius - min_tick_radius)
                                radius_interval = max(12, int(round(radius_span / max(len(r_ticks), 1))))
                                scale_factor = 3
                                amplifier_counts = 3
                                item_key = f"{entity_name}_{axis_label}"
                                safe_name = re.sub(r'[^A-Za-z0-9_.-]+', '_', item_key)
                                amplifier_prompt = self.generate_prompt(item_key, 'amplifier', dataset)

                                grid_tick = coords[0] if coords and coords[0] is not None else value
                                feedback_seed = feedback_tick[-1] if feedback_tick and feedback_tick[-1] is not None else grid_tick
                                amp_states = {
                                    "feedback": {"tick": feedback_seed, "dir": self.amplifier_feedback_image_dir, "ticks": []},
                                    "grid": {"tick": grid_tick, "dir": self.amplifier_grid_image_dir, "ticks": []},
                                }

                                for amp_count in range(amplifier_counts):
                                    for amp_kind, state in amp_states.items():
                                        curr_tick = state["tick"]
                                        curr_radius = int(round(arg_a * curr_tick + arg_b)) if curr_tick is not None else 0
                                        inner_radius = max(0, curr_radius - radius_interval)
                                        outer_radius = min(max_radius + radius_interval, curr_radius + radius_interval)
                                        if outer_radius <= inner_radius:
                                            outer_radius = min(max_radius + radius_interval, inner_radius + radius_interval)

                                        print(f"当前处理: {entity_name} - {axis_label}, {amp_kind} amp_{amp_count}, 半径范围: {inner_radius}-{outer_radius}")
                                        amplifier_image_path = os.path.join(
                                            state["dir"],
                                            f'{chart_id}_{grid_type}_{safe_name}_amp_{amp_count}.png'
                                        )
                                        amplifier_image = self.crop_axis_label_region(
                                            amplifier_path, center_x, center_y, target_angle,
                                            outer_radius, angle_width, inner_radius, 30,
                                            scale_factor, r_ticks, arg_a, arg_b
                                        )
                                        if amplifier_image.size <= 0:
                                            print(f"警告: 无法保存图像 {amplifier_image_path}，因为裁剪区域为空")
                                            continue
                                        ...
                                        amplifier_coords = self.call_llm_response(
                                            amplifier_prompt,
                                            amplifier_image_path,
                                            item_key,
                                            dataset
                                        )
                                        if amplifier_coords and amplifier_coords[0] is not None:
                                            state["ticks"].append(amplifier_coords[0])
                                            state["tick"] = amplifier_coords[0]
                                        elif state["ticks"]:
                                            state["ticks"].append(state["ticks"][-1])

                                feedback_amp_ticks = amp_states["feedback"]["ticks"]
                                grid_amp_ticks = amp_states["grid"]["ticks"]
                                if feedback_amp_ticks:
                                    self.results_by_image[chart_id]['data'][entity_name][axis_label]['amplifier_feedback_ticks'] = feedback_amp_ticks
                                if grid_amp_ticks:
                                    self.results_by_image[chart_id]['data'][entity_name][axis_label]['amplifier_grid_ticks'] = grid_amp_ticks
                                    self.results_by_image[chart_id]['data'][entity_name][axis_label]['amplifier'] = grid_amp_ticks[-1]
                            except StopIteration:
                                pass
                            except Exception as e:
                                print(f"❌ 多重放大处理异常: {e}")
    ...
    def save_results(self, output_path: str = None) -> None:
        ...
        if output_path is None:
            ...
            output_path = f'coordinates_by_image_{chart_type}_{self.model_name}.json'
...
if __name__ == '__main__':
    ...
    # 指定要处理的单个JSON文件路径
    json_file_path = './backend/data/polar/output/radar/result/radar_001_evalution_datasets.json'
