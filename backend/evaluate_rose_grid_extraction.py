...
from demo_radar.encrypt_radar import RadarChartEncoder  # noqa: E402 — kept for algorithm_tick_mapping compatibility

from demo_rose.encrypt_rose import (  # noqa: E402
    detect_rose_circles as detect_rose_grid_circles,
    rose_radius_peaks,
)

# Reuse radar helper functions
from evaluate_radar_grid_extraction import (  # noqa: E402
...
ROSE_SYNTH_DIR = ROOT / "data" / "output" / "axis_sample_selection" / "radar_rose_50charts_20260628_140358" / "rose"
OUTPUT_DIR = ROOT / "data" / "output" / "rose_grid_eval"
...
def iter_dataset_jsons(dataset: str) -> list[Path]:
    ...
    paths = []
    for path in sorted(ROSE_SYNTH_DIR.glob("rose_*.json")):
...
def evaluate_one(json_path: Path, dataset: str, args: argparse.Namespace) -> EvalRow:
    ...
    try:
        ...
        axis_salvage = dataset == "real" and chart_id in REAL_ROSE_AXIS_USABLE
        if axis_salvage:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset", choices=["real", "synth", "all"], default="all")
...
def main() -> None:
    ...
    for dataset_name in dataset_names:
        json_paths = iter_dataset_jsons(dataset_name)
        if args.only:
