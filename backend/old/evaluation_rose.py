...
def draw_angle_indicator(image, center_x, center_y, target_angle, radius, arc_color=(0, 0, 255), line_color=(0, 0, 255), 
    ...
    """
    在图像上绘制特定角度的扇形弧线和对应轴上的十字标记
    
    :param image: 要绘制的图像
    :param center_x, center_y: 圆心坐标
    :param target_angle: 目标角度（度）
    :param radius: 弧线半径
    :param arc_color: 弧线颜色，默认黄色
    :param cross_color: 十字标记颜色，默认红色
    :param arc_thickness: 弧线粗细
    :param cross_size: 十字标记大小
    :param arc_angle_width: 弧线覆盖的角度范围（度）
    :return: 绘制后的图像
    """
    # 计算弧线的起始和结束角度（OpenCV中角度是顺时针从水平线开始计算的，需要转换）
    # 注意：在OpenCV中，0度指向右侧，角度顺时针增加
    start_angle =  - target_angle - arc_angle_width // 2
    end_angle =  - target_angle + arc_angle_width // 2  
    
    # 绘制黄色扇形弧线
    cv2.ellipse(image, (center_x, center_y), (radius, radius), 0, start_angle, end_angle, arc_color, arc_thickness, lineType=cv2.LINE_AA)
    
    #将角度转换为弧度并计算线段的端点坐标
    angle_rad = math.radians(target_angle)
    outer_x = int(center_x + (radius+line_length_ratio*radius) * math.cos(angle_rad))
    outer_y = int(center_y - (radius+line_length_ratio*radius) * math.sin(angle_rad))  # 注意图像坐标系中y轴向下，所以减去
    ...
    # 在轴的端点绘制一个小的十字标记，方便识别
    # cross_size = 5
    # # 水平线
    # cv2.line(image, (outer_x - cross_size, outer_y), (outer_x + cross_size, outer_y), line_color, line_thickness, lineType=cv2.LINE_AA)
    # # 垂直线
    # cv2.line(image, (outer_x, outer_y - cross_size), (outer_x, outer_y + cross_size), line_color, line_thickness, lineType=cv2.LINE_AA)
...
if __name__ == '__main__':
    #将数据集保存到JSON文件
    #生成数据集的prompt
    with open(f'evaluation_datasets_with_axes_double.json', 'r', encoding='utf-8') as f:

        datasets = json.load(f)
   
    feedback_image_dir = './data/feedback'
    amplifier_image_dir = './data/amplifier/rose'
    ...
    for grid_type in ['with_grid','no_grid']:
        for dataset in datasets[:20]:
            if dataset['chart_type'] != 'rose' or not dataset['data']:
                continue
            ...
            for key, value in dataset['data'].items():
                item_name = key  # 直接使用键作为item_name
                ...
                if(grid_type == 'with_grid'):
                    ...
                    while(feedback_counts):
                        ...
                        feedback_image = draw_angle_indicator(feedback_image, center_x, center_y,target_angle,pre_r,line_thickness=2,arc_angle_width=angle_width,line_length_ratio=0.05)
                ...
                if(grid_type == 'with_grid'):
                    ...
                    amplifier_image = crop_axis_label_region(amplifier_path, center_x, center_y, target_angle, outer_radius, angle_width, inner_radius,30, scale_factor,r_ticks,arg_a,arg_b)
...
# 您正在分析一张玫瑰图的一部分。该图通过扇形的**最远端半径**来展示数据，其颜色边界对应的数值为数据值
# 以下为图表的详细信息：
# 您的任务是估计标记为"{item_name}"对应的扇形的值，即找到扇形的**最远端半径**的颜色边界，获取其相对于网格基准线的对应值
# -请先找到{item_name}对应颜色为{dataset.get('series_color', {}).get(item_name.split(',')[0].strip(), '未知颜色')}，**并且找到其最远端的边界**
# -然后找到该边界对应的哪两个基准线之间
# -最后依据基准线的数值，插值出数值
# **注意区分网格线和扇形的边界，我们需要获取的扇形边界为实线，而非虚线**